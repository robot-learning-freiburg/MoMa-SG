...
class KinectRGBDDataset(Dataset):
    """KinectRGBDDataset class.
    ...
    def __init__(self, cfg):
        """Initialize the dataset: index RGB/depth frames, load camera parameters/poses/TF
        calibration, and read interaction-segment annotations.

        Args:
            cfg (dict): Configuration dictionary. Expected keys include "root_path" (Path to
                the scene directory containing "rgb"/"depth"/"odom" subfolders) and
                "tf_file_path" (path to the depth/base TF calibration JSON), plus optional
                "depth_min", "depth_max", "flipped", "gt_poses", and "droid_slam".
        """
    ...
    def __getitem__(self, idx):
        ...
        The pose is optionally corrected by a DroidSLAM registration matrix, remapped from
        depth-frame to RGB-frame via the TF calibration (for ground-truth poses), and/or
        corrected for a 180-degree yaw flip of the input images.
        ...
        if self.gt_poses:  # poses generated from the HTC tracking system
            assert self.droid_slam is False, "Cannot use both gt_poses and droid_slam simultaneously."
            pose = pose @ self.base_T_depth @ self.depth_T_rgb
        if self.flipped and not self.droid_slam:  # input is flipped
            T_flip = np.eye(4)
            T_flip[:3, :3] = R.from_euler("z", 180, degrees=True).as_matrix()
            pose = pose @ T_flip
    ...
    def _load_parameters(self):
        """Load camera intrinsics, camera poses (from ground-truth or DroidSLAM), the depth/base
        TF calibration, and the scene's ground-truth articulation file, populating the
        corresponding instance attributes. Raises if the TF calibration file is missing.
        """
        # Depth and RGB camera intrinsics are the same for the Kinect
        ...
        if Path(os.path.join(self.root_path, Path(self.root_path).stem + ".json")).exists():
            ...
            loguru.logger.info(f"Loaded scene articulation file:{os.path.join(self.root_path, Path(self.root_path).stem + '.json')}")
        else:
    ...
    def get_camera_poses_aligned(self):
        """Compute the (registration/TF/flip-corrected) camera-to-world pose for every frame in
        the dataset, using the same pose logic as `__getitem__`.

        Returns:
            list[np.ndarray]: List of 4x4 homogeneous camera-to-world transforms, one per frame,
                in dataset order.
        """
        camera_poses = []
        for idx in tqdm.tqdm(range(len(self.data_list)), desc="Getting camera poses", total=len(self.data_list)):
            rgb_path, _ = self.data_list[idx]
            timestamp = float(Path(rgb_path).stem.split("_")[-1])
            pose = self._get_closest_pose_in_se3(timestamp) if self.cam_poses else np.eye(4)
            if self.droid_slam:
                if self.registered:
    ...
    def _read_tf(self):
        """Read the depth/base TF calibration JSON and populate `self.depth_T_rgb` and
        `self.base_T_depth` as 4x4 homogeneous transforms. Raises an Exception if the file is
        malformed or missing the required "depth_T_rgb"/"base_T_depth" rotation/translation
        entries.
        """
        with open(self.tf_file_path, "r") as f:
            try:
                ...
                if (
                    ...
                    or not all(key in tf["depth_T_rgb"] for key in ["rotation", "translation"])
                    or not all(key in tf["base_T_depth"] for key in ["rotation", "translation"])
                ):
                ...
                self.depth_T_rgb[:3, :3] = R.from_quat(quat).as_matrix()
                ...
                self.base_T_depth[:3, :3] = R.from_quat(quat).as_matrix()
